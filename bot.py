...
@bot.tree.command(name="initialize", description="First thing to do after adding the bot", guild=discord.Object(id=TESTING_ENV_ID))
async def init(interaction: discord.Interaction):
    ...
    if not interaction.guild:
        ...
        ai_channel = await interaction.guild.create_text_channel(name="ai-chat", reason="Initialized Command", overwrites=overwrites)
        log_channel = await interaction.guild.create_text_channel(name="ai-logs", reason="Initialized Command", overwrites=overwrites)
        res = initialize_guild(guild_id=interaction.guild_id, log_channel_id=log_channel.id, ai_channel=ai_channel.id)
        print(res["message"])
